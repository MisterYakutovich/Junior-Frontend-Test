...
import { GET_CATEGORIES } from "../../components/Header/categories_query";
...
class Header extends React.Component {
    constructor(props){
        ...
        this.toggleVisibleCurrency=this.toggleVisibleCurrency.bind(this)
    ...
    onSelectedItem=(index)=>{
        ...
        this.toggleVisibleCurrency(false)  
    }
   
    static contextType = Context;
    render() {     
      const {arr,onCloseOverlay,deleteProdukt,openOverlay}=this.props;  
      const categories=this.state.categories.map(i=>i.name)
       
    return(
        <header  className="header">
                     
                  {this.state.openCurrency && <Currency isActiv={this.state.isActiv} 
                                                          onSelectedItem={(index)=>this.onSelectedItem(index)}  
                                                          toggleVisibleCurrency={this.toggleVisibleCurrency}
                                                         /> }
                              {openOverlay && <CartOverlay arr={arr}
            ...
            <nav className="header_nav">    
                <ul className="hr">   
                    <div className="container2">
                        <li>
                        <a href="/clothes" className="header_list">{categories[1]}</a>
                        </li>
                        </div>
                    <div className="container3"> 
                        <li> 
                           <a href="/technology" className="header_list1">{categories[2]}</a>
                        </li>
                        </div>
                    <div className="container4">
                        <li>
                            <a href="/all_product" className="header_list2">{categories[0]}</a>
                            </li>
                            </div>                  
                </ul>                   
            </nav>
        ...
        <div  className="header_menu">
            <div className="total_index">
                <b className="total_index_number">{arr.length}</b>
            </div>
            <div onClick={this.props.onClickCart} className="header_menu_basket">
            ...
            <div className="header_menu_currency">
                <div className="header_menu_currency_switch">{this.state.isActiv}
                    
    </div>
                   <div onClick={this.toggleVisibleCurrency} className="header_menu_vector">
